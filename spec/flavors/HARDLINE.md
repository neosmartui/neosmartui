...
The deterministic Foundry builder extends the **deployed artifact** for that same canonical route: it copies the proven Light source page, injects the generated `hardline-dark-theme.css` link, injects `apps/foundry/fragments/hardline-dark.html` before the route's existing return link, and writes only the artifact copy under `dist/foundry/flavors/hardline/index.html`. The deployed Light section remains `.ns-theme-hardline-light`; the injected Dark section is scoped by `.ns-theme-hardline-dark` and consumes the same `.ns-button`, `.ns-input`, `.ns-card`, and `.ns-badge` adapter classes. The build emits `hardline-theme.css` and `hardline-dark-theme.css` from the two independent resolved bundles.
