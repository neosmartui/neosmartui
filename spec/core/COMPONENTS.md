...
`core.combobox` enters Core at `contract-only` maturity as the first editable suggestion-list selection composite. It is deliberately distinct from `core.select`: Select keeps a browser-owned picker and no portable open state, while Combobox owns editable query text, an explicitly related suggestion popup, transient active-option navigation, and one committed selected value.

The initial scope is single-selection only. Query text and committed selection remain different authorities; typing or moving highlight cannot silently claim a new selected value. Multi-select, chips/tags, freeform value creation, async request policy, virtualization, remote pagination, ranking, and business-specific filtering remain outside this slice.

The Web semantic contract keeps DOM focus on the editable combobox control while an open listbox exposes the active suggestion through `aria-activedescendant` or an equivalent established relationship. ArrowDown/ArrowUp may move active suggestion, Enter may commit an enabled active suggestion, Escape closes the popup without erasing a committed value or moving focus, and Tab/Shift+Tab retain ordinary document traversal. Home/End and Left/Right remain native text-editing keys by default rather than being stolen for list navigation.

Combobox is non-modal. Its suggestion popup MUST NOT trap focus, inert surrounding content, or masquerade as a Dialog/Command Menu/generic Popover. Empty-result messaging is non-interactive rather than a fake selectable option, and highlighted navigation state remains distinct from persistent selection.

The physical model also separates responsibilities. The editable text surface remains an editing surface and is not pressure-translated when the user places a caret or selects text. An optional real disclosure trigger follows the permanent pressure-not-levitation law. The suggestion list uses generic panel/surface roles, while trigger/input geometry uses control roles; this prevents the popup from borrowing control geometry merely for convenience.

The contract introduces no new semantic token contracts. All editable-control, popup-surface, focus, selected-option, invalid, disabled, pressure, and motion roles already exist among the 63 Core token contracts. Because Combobox remains contract-only, Rivet Light scope, all concrete Theme values, and the exact implemented/public dependency union of 55 remain unchanged.

Pinned migration authority is intentionally narrow and honest: family conformance provides generic composite/accessibility laws plus the Menus/Popovers trigger-compression rule; pinned Soft explicitly lists `Combobox surfaces` while also identifying keyboard-complete production Combobox behavior as unfinished; pinned Rivet provides reference-only `components/ui/combobox.tsx` anatomy. NeoSmartUI does not claim a completed Soft runtime or copy Rivet source code, and narrows Rivet's optional chips/multi-select knowledge out of this first contract.

Registry maturity is `contract-only`. Implementation evidence and public-proof evidence are both `null`; no Web adapter, Theme resolution change, Foundry demo, browser-test expansion, workflow change, or deployment/proof promotion is part of Slice 17.
