...
`core.accordion` is admitted as a contract-only generic disclosure composite. It owns repeated heading/trigger/panel relationships for revealing and collapsing related content without becoming Tabs, navigation, a menu, or a business-domain workflow.

Each item uses a real button trigger with `aria-expanded` and `aria-controls`, plus a stable controlled panel associated back to the trigger. The root receives no synthetic widget role. Panel `role="region"` remains optional rather than automatic so a long Accordion does not create landmark noise.

The state model is exactly `rest`, `hover`, `focus-visible`, `pressed`, `closed`, `open`, and `disabled`. Open/closed is disclosure state, not selection. Tab and Shift+Tab remain ordinary document traversal, Space/Enter use native button activation, and Core Accordion does not require ArrowUp/ArrowDown/Home/End remapping or a roving-tabindex model. Opening a panel does not automatically move focus into it, while hidden descendants must not remain operable as if visible.

The trigger follows the permanent pressure-not-levitation law: rest keeps structural depth, hover/contact compresses toward the surface, press reaches active depth, and release restores depth. Disclosure state persists independently of the transient press. Panel reveal may communicate hierarchy but must not lift the trigger, and reduced motion makes disclosure near-instant without changing semantics.

The contract reuses existing control, panel/surface, focus, minimum-target, depth/press, disabled-opacity, typography, and semantic motion roles. It introduces no Accordion-specific token and no Theme value. Because `core.accordion` remains `contract-only`, Rivet Light scope and the exact implemented/public dependency union remain unchanged at 55, while semantic token contracts remain 63.

Migration authority is intentionally narrow: selected evidence grows from 58 to 60 through `legacy.spec.accordion-conformance` and `legacy.rivet.accordion-anatomy`. The family snapshot contributes generic explicit-state, pressure, focus, keyboard, touch, reduced-motion, semantic-token, and agent-readable laws but contains no separate Accordion-specific law. Pinned Rivet contributes root/item/header-trigger/content anatomy and open/closed, disabled, focus, and indicator knowledge from `components/ui/accordion.tsx`; it is reference-only and no source code is copied. Pinned Soft does not list or implement Accordion, so no Soft Accordion provenance is claimed.

Registry maturity is `contract-only`; implementation evidence and public-proof evidence are both `null`. The existing 17 public proofs remain bound to the deployed `e492fc06fd14d22087c0b428d2788d3d9824212a` singleton cohort, and the Chromium suite remains exactly 90 cases in this contract phase. No Web adapter, Theme bundle/value, Foundry source, browser test, proof record, or deployment changes belong in this PR. A canonical implementation is the next lifecycle stage only after this contract merges green and independently reproduces on merged main.
