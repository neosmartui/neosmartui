...
`core.tooltip` is admitted as a contract-only Core primitive for concise supplemental descriptions. It is not a popup action surface, not a replacement for an accessible name, not a validation/error channel, and not a place for essential task instructions. The described target keeps its own semantics while the supplemental surface uses `role="tooltip"` and an explicit description relationship such as `aria-describedby`.

The contract exposes exactly `closed`, `open`, and `dismissed`. Hover and keyboard focus can reveal the same supplemental content when the target already supports those input modes. Pointer users may move from the target onto the tooltip without immediate disappearance, while keyboard focus remains on the target because Tooltip itself is non-interactive and never enters the Tab order.

Escape dismisses an open Tooltip without moving focus or activating the target. Dismissal persists for the current uninterrupted hover/focus session so the same condition cannot immediately reopen it; a later genuine leave/re-enter or focus-away/focus-back session may reveal it again. Tooltip never traps focus and does not intercept Tab, Shift+Tab, Enter, Space, Arrow keys, Home, or End for the target.

The semantic boundary forbids interactive descendants. Buttons, links, form controls, menus, dismiss controls, or richer user-controlled persistence require a Popover/Dialog-class capability instead. Core also does not make arbitrary static content focusable solely to gain a Tooltip trigger.

Touch support is deliberately non-invasive. Because touch has no dependable hover model, essential information cannot be available only through Tooltip. Implementations must not hijack tap, long-press, context-menu, text selection, or native activation merely to reproduce desktop hover behavior; persistent text or a touch-appropriate disclosure surface owns that requirement instead.

The permanent family interaction rule is explicit: Tooltips must not introduce movement that makes the target appear actionable when it is not. Showing or hiding the supplemental surface therefore never translates, lifts, compresses, scales, or deepens the target. A restrained non-spatial reveal such as opacity may communicate hierarchy, but decorative travel, bounce, spring, hover lift, and target-following pressure are outside the Core contract.

The dependency boundary intentionally reuses existing informational-surface roles only: panel/content/border colors, annotation padding/border/radius, resting structural depth, standard non-physical motion timing, and body typography. Tooltip does not borrow target size, focus ring, control geometry, action colors, hover/active depth, press translation, press/release timing, or disabled opacity. This contract-only slice changes no Theme resolution scope or concrete Theme value.

Pinned migration authority consists of four selected artifacts: the family Tooltip movement law, Soft's explicit reusable Tooltip capability, Soft's `aria-describedby` plus `role="tooltip"` anatomy, and Soft's compact bounded visual surface. These remain knowledge/reference evidence only; no source code is copied. The complete pinned Rivet tree was reviewed and contains no `components/ui/tooltip.tsx`, so NeoSmartUI makes no Rivet Tooltip implementation claim.

Registry maturity is `contract-only`; implementation and public-proof evidence both remain `null`. This slice therefore adds no Tooltip runtime CSS/JavaScript, Theme scope/value changes, Foundry markup, deployable files, public proof, or browser tests. Those belong to later lifecycle stages only after this contract merges green.
