...
`core.segmented-control` is now implemented as a compact single-selection mode/view control. It is a visually connected set of real peer buttons where exactly one persistent mode is active through `aria-pressed="true"`; it is not a submitted form-value choice and not a tab-panel composite.
...
Registry maturity is `implemented`, with canonical implementation evidence `packages/adapters/web/components/segmented-control.mjs` and public-proof evidence still `null`. All fourteen previously public-proof components remain bound to their unchanged singleton deployment cohort until an exact merged-main artifact containing Segmented Control is deployed and the singleton proof cohort is refreshed.
