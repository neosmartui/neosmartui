...
Rivet Light now resolves exactly the two roles introduced by the contract: `space.navigation.gap` is `0.45rem`, matching pinned Soft breadcrumb spacing, and `font.size.navigation` is `clamp(0.72rem, 0.69rem + 0.08vw, 0.78rem)`, matching pinned Soft text-xs typography. Every previously resolved Theme value remains unchanged. Adding Breadcrumb to implemented Theme scope expands the exact dependency union from 53 to 55.
...
Registry maturity is `implemented`, with canonical evidence `packages/adapters/web/components/breadcrumb.css` and `publicProof: null`. Public-proof promotion remains a separate lifecycle step after this exact implementation is merged, reproduced on merged-main CI, deployed byte-for-byte, and verified on the singleton public host.
