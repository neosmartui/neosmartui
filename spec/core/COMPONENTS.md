...
`core.pagination` is now implemented as generic navigation across discrete pages in one ordered collection. Real destinations remain real anchors, exactly one page is identified as current with `aria-current="page"`, and routing, URL serialization, fetching, total-count discovery, page-size policy, virtual scrolling, and infinite-scroll behavior remain outside Core Pagination.
...
Pagination introduces no new token contracts or Theme values. Rivet Light scope now includes `core.pagination`, but every declared dependency was already resolved by the existing public Core cohort, so the token-contract count remains 63 and the exact implemented/public dependency union remains 55. The implementation deliberately reuses `space.navigation.gap` and `font.size.navigation` with existing control geometry, depth/press, focus, surface/action, disabled-opacity, and emphasis roles rather than inventing pagination-specific tokens.
...
Registry maturity is `implemented`, with canonical implementation evidence `packages/adapters/web/components/pagination.css` and public proof still null. This implementation slice changes no existing proof record; all 13 public-proof components remain bound to the current singleton deployment cohort until Pagination separately passes merge, exact merged-main artifact deployment, and proof-cohort refresh gates.
