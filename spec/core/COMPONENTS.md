...
`core.field` starts contract-only as the generic composition around one primary form control. It closes the label/help/error boundary deliberately left outside `core.input`, `core.textarea`, and `core.select`: those primitives continue to own native editing, selection, activation, focus, value, invalid, disabled, and read-only behavior, while Field owns the readable and programmatic relationships around one control.

The visible label must be validly associated with the primary control; placeholder text, help copy, headings, or visual proximity do not substitute for that relationship. Supporting description/help and validation text must be linked to the control with `aria-describedby`, `aria-errormessage`, or another valid platform mechanism when their meaning applies. Existing author-supplied description relationships must be preserved rather than overwritten.

A simple one-control Field does not manufacture `role="group"`, and static validation text does not automatically receive `role="alert"`. Grouping and announcement semantics depend on the real composition and timing. Multiple related controls belong to an explicit grouping capability such as native `<fieldset>`/`<legend>` rather than silently widening this primitive.

Machine-readable invalidity, required/optional meaning, and disabled/read-only behavior remain authoritative on the real control. Field may reflect invalid or disabled presentation, but the container itself does not disable, focus, activate, compress, lift, or become a second interaction target. Nested links or actions remain independent components with their own accessibility and tactile contracts.

Field introduces one value-free semantic spacing role, `space.field.gap`, for the block-axis relationship between label, control, help/description, and validation message. It deliberately does not borrow `space.control.*`, surface spacing, annotation geometry, target-size, depth, focus-ring, or press/motion roles. No concrete Theme value is introduced at contract-only maturity.

Pinned Soft explicitly classifies `label/help/invalid field` as Core, and its pinned Field wrapper plus input CSS provide useful anatomy, invalid-presentation, and dedicated-gap knowledge. Pinned Rivet independently provides Field/Label/Description/Error plus fieldset/legend anatomy and orientation knowledge. NeoSmartUI treats both as reference-only: Soft's wrapper-label shape is not assumed to fit every composition, and Rivet's generic `role="group"` Field host plus unconditional `role="alert"` FieldError are not adopted as defaults. No legacy source code is copied.

Registry maturity is `contract-only`; implementation and public-proof evidence remain null. No Web adapter/CSS implementation, Rivet Light token value, Theme scope expansion, Foundry example, browser test, or public proof is claimed by this contract slice.
