...
if (!tooltipEntry || tooltipEntry.maturity !== 'implemented') fail('core.tooltip must be implemented in this slice');
...
if (tooltipEntry.evidence.publicProof !== null) fail('implemented core.tooltip must not claim public proof before deployment');
...
for (const marker of ['Tooltip content is supplemental, not essential', '`role="tooltip"`', '`aria-describedby`', 'MUST NOT contain buttons, links, inputs, menus, dismiss controls', 'Escape dismisses it without moving focus from the target', 'MUST NOT trap focus', 'Tooltips MUST NOT introduce movement that makes the target appear actionable', 'MUST NOT hijack tap, long-press, context-menu, text-selection, or native activation behavior', 'contains no `components/ui/tooltip.tsx`', 'exact dependency union therefore remains 55', '`packages/adapters/web/components/tooltip.css`', '`packages/adapters/web/components/tooltip.mjs`', 'Maturity is `implemented`']) if (!tooltipDocs.includes(marker)) fail(`tooltip.md missing marker: ${marker}`);
