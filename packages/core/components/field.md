...
Visual reordering MUST NOT separate label/help/error content from the reading order expected by keyboard and assistive-technology users.
...
Forced-colors/high-contrast rendering must preserve readable label/help/error text. Invalid meaning cannot depend on custom red alone; machine-readable invalidity plus explicit validation text remain the authoritative channels.
...
Contract-only maturity adds no concrete Theme value for `space.field.gap`. Flavor/Theme resolution chooses that value only when an implementation is admitted, and existing token values MUST NOT be mutated merely to make Field fit.
...
- Soft implementation knowledge: the same pinned repository's `registry/default/field/field.tsx` and `src/components/input.css` demonstrate label/control/help anatomy, invalid presentation, and a dedicated field gap. The legacy wrapper-label approach is not treated as universally correct for arbitrary composition; NeoSmartUI requires an explicit valid label/control relationship instead.
...
Maturity is `contract-only`. No Web implementation, Theme value for `space.field.gap`, Foundry example, browser test, or public-proof claim exists in this slice.
