...
Alert content may contain multiple sentences, lists, links, or controls when the message requires them. Core MUST NOT force one-line truncation or a fixed height that hides essential localized or validation content.
...
Forced-colors/high-contrast rendering must preserve readable text and a perceivable message boundary. Tone meaning cannot disappear when custom backgrounds or status colors are overridden; non-color content remains authoritative.
...
Contract-only maturity introduces no new token contracts and no Rivet Light values. A future implementation must resolve only the existing dependency union required by its chosen surface/tone treatment and must not mutate established values merely to make Alert visually distinct.
